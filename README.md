...
> ⚠️ **Warning**: You may need to manually copy the required DLL files from `C:\...\strawberry_perl\c\bin`. The build process loads these libraries and expects their exact filenames. A common symptom of this issue is that the program appears to start but no window opens, and the process just sits idle in Task Manager.
